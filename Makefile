...
laminar-up: laminar-env ## Start self-hosted Laminar
	cd "$(REPO_ROOT)" && docker compose --env-file "$(LAMINAR_ENV)" -f "$(LAMINAR_COMPOSE)" up -d
...
laminar-check: laminar-env ## Validate compose and local Laminar endpoints
	...
	@cd "$(REPO_ROOT)" && set -a; . "$(LAMINAR_ENV)"; set +a; curl -sS --max-time 3 "http://localhost:$${FRONTEND_HOST_PORT:-5667}" >/dev/null && echo "Laminar frontend reachable: http://localhost:$${FRONTEND_HOST_PORT:-5667}" || (echo "Laminar frontend is not reachable. Run 'make laminar-up' first." >&2; exit 1)
	@cd "$(REPO_ROOT)" && set -a; . "$(LAMINAR_ENV)"; set +a; curl -sS --max-time 3 "http://localhost:$${APP_SERVER_HOST_PORT:-8000}" >/dev/null && echo "Laminar app-server reachable: http://localhost:$${APP_SERVER_HOST_PORT:-8000}" || (echo "Laminar app-server is not reachable. Run 'make laminar-up' first." >&2; exit 1)
