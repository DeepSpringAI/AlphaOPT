.PHONY: help sync sync-extras train eval check-proxy train-with-proxy smoke
...
sync-extras: ## Install with optional extras (notebooks, retrieval, solvers)
	cd "$(REPO_ROOT)" && $(UV) sync --all-extras

smoke: ## Quick import check for core modules
	cd "$(REPO_ROOT)" && $(UV) run python -c "import src.utils; import src.credentials; print('ok')"
