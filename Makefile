...
laminar-check: laminar-env ## Validate compose and local Laminar endpoints
	...
	@cd "$(REPO_ROOT)" && set -a; . "$(LAMINAR_ENV)"; set +a; curl -sS --max-time 10 "http://localhost:$${APP_SERVER_HOST_PORT:-8000}" >/dev/null && echo "Laminar app-server reachable: http://localhost:$${APP_SERVER_HOST_PORT:-8000}" || (echo "Laminar app-server is not reachable. Run 'make laminar-up' first." >&2; exit 1)
